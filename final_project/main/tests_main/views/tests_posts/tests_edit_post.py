...
class EditPostViewTests(ValidPostData, UserAndProfileData, django_test.TestCase):
    EXPECTED_TEMPLATE = 'posts/edit_post.html'
    ...
    def __get_response_for_profile(self, post):
        return self.client.get(reverse('edit post', kwargs={'pk': post.pk}))
    ...
    # CHECK IF VIEW DELETES POST SUCCESSFULLY WITH CORRECT CREATOR
    def test_view_edits_post_correctly(self):
        user, profile = self.__create_valid_user_and_profile()
        self.client.login(**self.VALID_USER_CREDENTIALS)
        post = self.__create_post_view(user)
        self.client.post(
        ...
        changed_title = 'editedtitle'
        changed_description = 'post description'
        post = Post.objects.get(pk=post.pk)

        self.assertEqual(changed_title, post.title)
        self.assertEqual(changed_description, post.description)
